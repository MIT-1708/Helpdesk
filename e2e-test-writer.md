...
- **E2E Spec Location**: Tests are written in the `e2e/` directory: [auth.spec.ts](file:///c:/Users/allle/OneDrive/Desktop/Helpdesk/e2e/auth.spec.ts) (Authentication & RBAC) and [tickets.spec.ts](file:///c:/Users/allle/OneDrive/Desktop/Helpdesk/e2e/tickets.spec.ts) (Ticket Assignment & API Validation).
- **Test Artifacts and Reports**: Configured in [playwright.config.ts](file:///c:/Users/allle/OneDrive/Desktop/Helpdesk/playwright.config.ts) to output test run details to `e2e/test-results/` and HTML reports to `e2e/playwright-report/`.
- **Scripts**:
